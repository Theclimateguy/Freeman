"""Command-line interface for Freeman librarian."""
...
from freeman_librarian.agent.costmodel import BudgetLedger, build_budget_policy, budget_tracking_enabled
from freeman_librarian.game.runner import SimConfig
from freeman_librarian.interface.api import InterfaceAPI
from freeman_librarian.interface.factory import (
...
from freeman_librarian.interface.identity import build_identity_state
from freeman_librarian.interface.kgevolution import KnowledgeGraphEvolutionExporter
...
from freeman_librarian.interface.simulationdiff import build_simulation_diff, export_simulation_diff
from freeman_librarian.llm import ExplanationRenderer, IdentityNarrator
from freeman_librarian.core.world import WorldState
from freeman_librarian.memory.knowledgegraph import KGNode, KnowledgeGraph
from freeman_librarian.memory.reconciler import Reconciler
...
from freeman_librarian.runtime.queryengine import RuntimeAnswerEngine, RuntimeQueryEngine, load_runtime_artifacts
...
DEFAULT_CONFIG: dict[str, Any] = {
    "agent": {
        ...
        "stream_keywords": [],
        "bootstrap": {
            "mode": "document_bootstrap",
            "schema_path": None,
            "policies_path": None,
            "documents_path": "./data/documents",
            "document_glob": "**/*",
            "fallback_schema_path": "./freeman_librarian/domain/profiles/gim15.json",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="freeman-librarian")
    ...
    bootstrap_docs_parser = subparsers.add_parser("bootstrap-docs")
    _add_config_option(bootstrap_docs_parser)
    bootstrap_docs_parser.add_argument("paths", nargs="*")
    bootstrap_docs_parser.add_argument("--domain-id", default="freeman_librarian")
    bootstrap_docs_parser.add_argument("--world-output-path")
    bootstrap_docs_parser.add_argument("--session-log-output-path")
    bootstrap_docs_parser.add_argument("--report-output-path")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "bootstrap-docs":
        bootstrap_cfg = config.get("agent", {}).get("bootstrap", {})
        document_paths = list(args.paths)
        if not document_paths:
            configured_root = bootstrap_cfg.get("documents_path")
            configured_glob = str(bootstrap_cfg.get("document_glob", "**/*"))
            if configured_root:
                resolved_root = _resolve_path(config_path.parent, configured_root, configured_root)
                document_paths = [
                    str(path)
                    for path in sorted(resolved_root.glob(configured_glob))
                    if path.is_file() and path.suffix.lower() in {".txt", ".md", ".rst", ".json", ".pdf", ".docx"}
                ]
        if not document_paths:
            raise RuntimeError("bootstrap-docs requires explicit paths or agent.bootstrap.documents_path in config.")

        bootstrapper = DocumentBootstrapper()
        result = bootstrapper.bootstrap_paths(
            document_paths,
            domain_id=args.domain_id,
            knowledge_graph=knowledge_graph,
        )

        if args.world_output_path:
            output = Path(args.world_output_path).resolve()
            output.parent.mkdir(parents=True, exist_ok=True)
            output.write_text(json.dumps(result.world_state.snapshot(), indent=2, sort_keys=True), encoding="utf-8")
        if args.session_log_output_path:
            result.session_log.save(args.session_log_output_path)
        if args.report_output_path:
            output = Path(args.report_output_path).resolve()
            output.parent.mkdir(parents=True, exist_ok=True)
            output.write_text(result.verification_report.to_json(), encoding="utf-8")

        _print_json(
            {
                "status": "completed",
                "mode": "document_bootstrap",
                "domain_id": result.world_state.domain_id,
                "document_count": len(result.extracted_documents),
                ...
                "verification": result.verification_report.snapshot(),
                "reconciliation": result.reconciliation_result.__dict__,
        ...
        _print_json(
            {
                ...
                "note": (
                    "Use `bootstrap-docs` to build an organizational world model from a document corpus, "
                    "or fall back to the inherited schema-based run path."
                ),
            }
        )
        return 0
