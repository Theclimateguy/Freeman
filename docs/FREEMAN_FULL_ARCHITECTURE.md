...
> Срез построен по текущему рабочему дереву ветки `hive_mind` на `2026-05-10`. Документ описывает реализованное поведение кода, включая runtime, CLI, MCP/tools, LLM-bootstrap, симулятор, память, consciousness layer, query/what-if surfaces и fallback-механизмы.
