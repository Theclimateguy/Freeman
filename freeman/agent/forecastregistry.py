"""Forecast registry with verification horizons and JSON persistence."""
...
class Forecast:
    ...
    def deadline_step(self) -> int:
        return int(self.created_step) + int(self.horizon_steps)
    ...
    @classmethod
    def from_snapshot(cls, data: dict[str, Any]) -> "Forecast":
    ...
    def all(self) -> List[Forecast]:
        return [
            ...
            for forecast in sorted(
                ...
                key=lambda item: (int(item.created_step), str(item.forecast_id)),
            )
        ]
